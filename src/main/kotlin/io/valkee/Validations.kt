...
object Validations {
    ...
    object Strings {
        fun <E, R> length(expected: ValkeeRule<Int?, Int?, E, R>) = rule(
            id = "validations.string.length",
            expectation = expected,
            property = ValkeeProperty<String?, Int?>("length", { it?.length }),
        ...
        fun equalsIgnoreCase(expected: String) = selfRule<String?, String>(
            id = "validations.string.equalsIgnoreCase",
            expectation = expected,
        ...
        fun matches(regex: Regex, message: String = "match {expectation}") = selfRule<String?, Regex>(
            ...
            message = MsgTpl(
                message = "{subject} {property} {value} {verb} $message",
                okVerb = "",
                nokVerb = "should"
            )
        )
    ...
    fun <S, V, E, R> context(
    ...
    ): (RuleCheckMessageTemplate.Context<S, V, E, R>) -> Map<String, *> =
        {
            ...
            mapOf(
                ...
                "result" to (
                    if (it.result is RuleCheck<*, *, *, *>) { it.result.message() } else {
                        display(it)
                    }
                    ),
